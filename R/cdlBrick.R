...
function( Dir, years, State, Fips, nodes, extent, setId = NULL) {
  ...
  for( index in which(cdl.resampleList ==T) ) {
    ...
    if( file.exists( sprintf('%sCDL_%s/CDL_%d_%02d_resampled.tif',Dir,State,year,Fips))) {
      ...
      if ( proj4string(cdl.reproj) != proj4string(cdl.baseRaster) ) { 
        ...
        cdl.reproj <- projectRaster( cdl.reproj, cdl.baseRaster,method='ngb', progress="text") 
        print(sprintf("Resampling year %d", year))
        cdl.resampled <- raster:::resample( cdl.reproj, cdl.baseRaster, method='ngb', progress="text") 
      } else {
        print(sprintf("Resampling year %d", year))
        cdl.resampled <- raster:::resample( cdl.reproj, cdl.baseRaster, method='ngb', progress="text") 
      }
      ...
      print(sprintf("Writing year %d, filename = %s", 
                    year,
                    sprintf('%sCDL_%s/CDL_%d_%02d_resampled.tif',Dir,State,year,Fips)
                    ))
